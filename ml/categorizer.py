...
class TransactionCategorizer:
    def __init__(self, user_id):
        ...
        self.base_data_path = os.path.join(settings.BASE_DIR.parent, 'ml', 'synthetic_data.csv')
    ...
    def _build_base_model(self):
        ...
        base_descriptions, base_categories = self._load_base_training_data()
        if base_descriptions and len(set(base_categories)) > 1:
            self.pipeline.fit(base_descriptions, base_categories)
            self._save_model()
    ...
    def train(self, descriptions, categories):
        ...
        if combined_descriptions and len(set(combined_categories)) > 1:
    ...
    def _load_base_training_data(self):
        try:
            ...
            if {'description', 'category'}.issubset(df.columns):
                clean = df[['description', 'category']].dropna()
                return clean['description'].astype(str).tolist(), clean['category'].astype(str).tolist()
